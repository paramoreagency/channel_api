...
if (! class_exists('Channel_data_lib')) {
    class Channel_data_lib
    {
        ...
        /**
         * Get an entry from a specified channel
         *
         * @param $entry_id
         * @param $channel_id
         * @param array $select
         * @return object
         */
        public function get_channel_entry_in_channel(
            $entry_id,
            $channel_id,
            $select = array(
                'channel_data.entry_id',
                'channel_data.channel_id',
                'channel_titles.author_id',
                'channel_titles.title',
                'channel_titles.url_title',
                'channel_titles.entry_date',
                'channel_titles.expiration_date',
                'status')
        )
        {
            $where = array(
                'channel_data.entry_id' => $entry_id,
                'channel_data.channel_id' => $channel_id
            );

            return $this->get_channel_entries($channel_id, $select, $where);
        }
            ...
            foreach ($fields as $field) {
                if (is_array($select)) {
                    switch ($field->field_type) {
                        case 'matrix':
                            $select[] = '\'' . $field->field_id . '\' as \'' . $field->field_name . '[matrix]\'';
                            break;

                        case 'playa':
                            $select[] = 'field_id_' . $field->field_id . ' as \'' . $field->field_name . '[playa]\'';
                            break;
        ...
        public function get_matrix_data($field_id, $entry_id)
        {
            ...
            return $this->EE->db->get('matrix_data')->result_array();
        }
        ...
        {
            return $this->EE->db->where('field_id', $field_id)->get('matrix_cols');
        }

        /**
         * This function extracts the entry_ids from the channel_data field value
         * based on the way that playa stores them.
         *
         * @param string $field_value The current value for the field from channel_data
         * @return array
         */
        public function extract_ids_from_playa_field_value($field_value)
        {
            $entry_ids = array();
